...
export function SmallToolDialog(props: { data: Tool; index: number; locations: ILocation[] }) {
  ...
  const current_location = (data.location_name || locations[0]?.name) ?? '';
  const current_condition = data.condition;

  const [conditionValue, setConditionValue] = useState<ToolCondition>(current_condition);
  ...
  // Ensure that brand, category, and tool_type are always strings
  const initialFormData: ToolFormData = {
    ...data,
    brand: data.brand ?? toolBrands[0] ?? '',
    category: data.category ?? toolCategories[0] ?? '',
    tool_type: data.tool_type ?? toolTypes[0] ?? '',
  };
  ...
  const handleEditClick = () => {
    if (isEditing) {
      // Save current state as initial form data for canceling edits
      setFormData(initialFormData);
    }
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="h-auto max-h-[90vh] overflow-auto border border-border bg-background lg:max-w-2xl">
        <DialogHeader>
